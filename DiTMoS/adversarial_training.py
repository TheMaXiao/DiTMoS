...
class DiTMoS_training_framework(nn.Module):
    def __init__(self, classifiers, selector, device):
        ...
        self.device = device
    #Define three regularization loss: union loss, overlap(intersection) loss, single loss.
    def intersection_union_loss(self, classifier_outputs, y, softmax_template):
        pred_value_list = []
        pred_correct_list = []
        ce_loss = nn.CrossEntropyLoss(reduction = "none")
        #select which classifier need to be trained
        ...
        #The items in pred_correct_list means whether it can be correctly classified.
        pred_value_list = torch.stack(pred_value_list, dim=1)
        pred_correct_list_to_tensor = torch.stack(pred_correct_list, dim=1)

        correct_num = torch.squeeze(pred_correct_list_to_tensor.sum(dim=1)) 
        ...
        #2. overlap_loss(multi_correct_loss)
        multi_correct_loss = []
        for i in range(self.num_classifiers):
        ...
        #3. single loss(one_correct_loss)
        one_correct_loss = []
        for i in range(self.num_classifiers):
